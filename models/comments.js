...
let getComments = (uuid) => {
  return new Promise((resolve, reject) => {
    ...
    let sql =
      "(SELECT * FROM comments as c1, comments as c2 WHERE c1.parent_uuid = ? AND c2.parent_uuid=c1.uuid)";
    connection.query(sql, [uuid, uuid], (error, results, fields) => {
      comments = results;
      connection.query(
        "SELECT * FROM comments WHERE parent_uuid = ?",
        [uuid],
...
let addComment = (comment, parent_uuid, username) => {
  return new Promise((resolve, reject) => {
    ...
    let sql =
      "INSERT INTO comments (comment_content, uuid, parent_uuid, username, likes, date_posted) VALUES (?, ?, ?, ?, 0, ?)";
    connection.query(
      sql,
      [comment, uuidv4(), parent_uuid, username, date],
      (error, result) => {
